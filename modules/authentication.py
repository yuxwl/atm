    ...
    def outer_wrapper(func):
                    ...
                    with open(__db_users_dict,"r+") as f_users_dict:
                        ...
                        if username in users_dict.keys():
                            if password == users_dict[username]["password"]:
                                ...
                                print ("\33[31;0m输入的密码不正确,认证搞失败\33[0m")
                        else:
                            print ("\33[31;0m输入的用户名不存在,认证失败\33[0m")
                else:
                    print ("\33[31;输入的用户名为空,认证失败0m\33[0m")
            return wrapper
        ...
        if auth_type == "admincenter_auth":
            def wrapper():
                ...
                username = input("\33[31;0m请输入管理用户名: \33[0m")
                password = input("\33[31;0m请输入管理密码: \33[0m")
                if len(username.strip()) >0:
                    if username in admincenter_dict.keys():
                        if password == admincenter_dict[username]:
                            ...
                            print ("\33[31;0m输入的密码不正确,认证失败\33[0m")
                    else:
...
def creditcard_auth():
    print ("\33[32;0m 信用卡登录认证\33[0m".center(40,"-"))
    return "True"

'''后台管理认证'''
@auth(auth_type="admincenter_auth")
def admincenter_auth():
    print ("\33[32;0m 后台管理登录认证\33[0m".center(40,"-"))
    return "True"